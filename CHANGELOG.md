...
### Added
- **Repair broken JSON**: `⋯ More → Repair JSON` in the editor takes the trailing commas, `//` and `/* */` comments, single quotes, unquoted keys, smart quotes, `NaN`/`Infinity`/`undefined` and unclosed brackets that real-world payloads arrive with, and proposes a fix. It never rewrites silently: the panel lists what it would change and waits for **Apply**, and the proposal retires as soon as the document changes so it cannot overwrite newer work. Content inside strings is never touched.
